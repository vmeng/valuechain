var JSON_SERVER = "";
var TESTING = false, allCategory = [];
networkStatus = "online";
// TESTING = true;
var categoryname = "", notesVisitTime, productNameArray, allProducts = [], previousPage, newsTopic, pushpinCompanyId = "", isClickPushpin = false, mapLoaded = false;
...
var app = {
	...
	addProductToCart : function() {
		...
		if (Number(item.num) == undefined || Number(item.num) == "" || Number(item.num) == 0) {
			...
			valueChain.tool.alertSucceed("Quantity " + item.num + ' of Product \"' + item.name + "\" is added to cart.");
		}
	},
	...
	deleteProductItem : function(e) {
		...
		try {
			confirm("Do you want to delete?", function(buttonIndex) {
	...
	initScroller : function() {
		...
		if (app.checkPlatformIpad()) {
			...
			for ( y = 0; y < feas.length; y++) {
				app.featureScroll[y] = new iScroll(feas[y], {
					hScrollbar : true,
					vScrollbar : true
				});
	...
	updateRevenue : function(id) {
		function onUpdate() {
			...
			osapi.jive.connects.get({
				alias : 'ws_all_revenue',
				headers : {
					'Accept' : ["text/xml; charset=\"utf-8\""]
				},
				href : '?customerAccId' + requestId
			}).execute(function(response) {
				 console.log('Jive service response');
		...
		if (ShoppingCart == null || ShoppingCart == "") {
			...
			var jsonstr = {
				"productlist" : [{
					...
					"num" : product.num,
					"price" : product.price,
					"quoted" : product.quoted,
					"orgQuoted" : product.orgQuoted,
					"pic" : product.pic
				}],
...
var sdc = {
	...
	recordMeeting : function() {
		...
		function onComplete() {
			...
			if (notesVisitTime) {
				...
				sdc.showMeetingNotes(app.getServerTime());
			sdc.refresh();
			sdc.currentMetting = {};
		};
	...
	alertNotesNotSave2 : function(data) {
		if ($("#input-meeting").val() != "") {
			try {
				confirm("Do you want to save the meeting notes?", function(buttonIndex) {
...
var contact = {
	...
	onNotesClick : function() {
		...
		if (notesVisitTime) {
			...
			sdc.go2ContactDetail(contact.currentContact, app.getServerTime());
	},
	...
	buildSocialFeeds : function(linkedInId, name) {
		...
		if (arr) {
			...
			$("#social-entry-wrapper").append(template({
				arr : arr
			}));
	...
	parseUpdateJsonSuccess : function(data) {
		...
		var activeLinkedInId = $(".cr66-active         .contact-personnel").attr("data-linkedin-id"), activeName = $(".cr66-active .cr-name").text();
