...
var app = {
    ...
    showSearchResult:function(content) {
        ...
        if (searchResult.length > 0) {
            ...
            $.each(searchResult, function(index, val) {
                ...
                $feed.find("img").attr("src", "../img/" + val.Item_Image[0]);
	...
	showCart:function(){
	    ...
	    for (i in productlist)
	    {
	        ...
	        var subqutoed =parseFloat((item.quoted *Number(item.num)).toFixed(2));
	            tag = "<tr class='cart_item'>" + ("<td><img src =' ../img/"+item.pic+"' width='40px' height='30px'></td>");
		...
		$.ajax({
			...
			success : function(data) {
				...
				$.each(data.categories, function(index, val) {
					...
					$feed.find("img").attr("src", "../img/" + val.avatar);
	...
	loadDetail : function(data) {
		...
		$("#quantity-num").val(1);
		$(".prod-pic").attr("src", "../img/" + data.Item_Image[0]);
...
var sdc = {
    ...
    go2ContactDetail : function(data,time) {
        ...
        $("#crm-person-headPic").attr('src', '../img/' + currentContact.Contact_Image);
...
var map = {
	...
	init : function(){
        ...
        controlText.style.background = 'url(../img/location.png) no-repeat';
...
var contact = {
	...
	buildContactDetail : function(){
		...
		$("#contact-avatar-a").attr("src","../img/"+contact.currentContact.Contact_Image);
	},
